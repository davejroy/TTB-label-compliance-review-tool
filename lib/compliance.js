...
function parsePercentAbv(value = '') {
  const match = value.match(/(\d{1,2}(?:\.\d+)?)\s*%/i);
  return match ? Number.parseFloat(match[1]) : null;
}

function extractPercentCandidates(labelText = '') {
  return Array.from(labelText.matchAll(/(\d{1,2}(?:\.\d+)?)\s*%\s*(?:ALC\.?\s*\/\s*VOL\.?)?/gi)).map((match) =>
...
function normalizeMeasure(value = '') {
  const match = value
    .toUpperCase()
    .replace(/\s+/g, ' ')
    .match(/(\d+(?:\.\d+)?)\s*(ML|L|FL\.?\s*OZ\.?|OZ\.?)/);

  if (!match) {
    return null;
  }

  return `${match[1]} ${match[2].replace(/\s+/g, ' ').replace(/\.$/g, '')}`;
}

function extractNetContentCandidates(labelText = '') {
  return Array.from(labelText.toUpperCase().matchAll(/(\d+(?:\.\d+)?)\s*(ML|L|FL\.?\s*OZ\.?|OZ\.?)/g)).map(
    (match) => `${match[1]} ${match[2].replace(/\s+/g, ' ').replace(/\.$/g, '')}`
  );
}
...
function compareAlcoholContent(labelText, expectedValue) {
  ...
  if (expectedAbv !== null && extractedAbvValues.length > 0) {
    const isMatch = extractedAbvValues.some((value) => Math.abs(value - expectedAbv) < 0.1);
