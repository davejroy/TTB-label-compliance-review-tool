...
>
> Development and testing take place in a separate development repository before changes are promoted here. This repository contains the full CI/CD pipeline for reviewed, production deployments.
>
...
- **`api.ts` Label-Only Check 404 (commit `1eea52f`).** `checkLabelsBatch` was
  calling `fetch()` with URL `${API_BASE}/label-check/batch` — missing the
  `/api` path segment — so every submission returned HTTP 404 Not Found and
  the UI hung until the request timed out. Fixed to `${API_BASE}/api/label-check/batch`.
  Also replaced the raw `fetch()` call with `safeFetch()` so transient 429/503/529
  errors are retried automatically, matching the behaviour of the other two API
  functions.
